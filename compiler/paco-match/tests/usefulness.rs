...
#[test]
fn irrefutable_struct_payload_covers_its_variant() {
    let arms = vec![
        arm(Pat::Enum {
            path: vec!["Shape".to_string(), "Rect".to_string()],
            fields: vec![Pat::Struct {
                path: vec!["Point".to_string()],
                fields: vec![("x".to_string(), Pat::Ident("x".to_string(), span()))],
                rest: true,
                span: span(),
            }],
            span: span(),
        ...
        arm(Pat::Enum {
            path: vec!["Shape".to_string(), "Rect".to_string()],
            fields: vec![Pat::Wildcard(span())],
            span: span(),
        }),
    ];

    let report = analyze_match(&arms, ConstructorSet::closed(["Shape::Rect"]));
