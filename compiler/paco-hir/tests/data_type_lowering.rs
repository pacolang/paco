...
fn hir_resolves_module_local_type_paths() {
    let module = parse_source("struct Point { x: i64 } fn make() -> Point { Point { x: 1 } }");
...
fn hir_records_type_references_from_declarations() {
    let module = parse_source(
        "struct Point { x: i64 } struct Line { start: Point } enum Maybe { Some(Point), None } fn shift(p: Point) -> Point { p }",
    );
...
#[test]
fn derivable_traits_records_a_derivable_attribute_s_own_fn_name() {
    let module = parse_source(
        "#[derivable(derive_display)]\ntrait Display { fn fmt(&self) -> string; }",
    );

    let derivable = paco_hir::derivable_traits(&module);

    assert_eq!(derivable.get("Display"), Some(&"derive_display".to_string()));
}

#[test]
fn a_trait_without_the_derivable_attribute_is_not_recorded() {
    let module = parse_source("trait Display { fn fmt(&self) -> string; }");
