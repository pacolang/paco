...
#[test]
fn check_allows_move_inside_loop_when_reinitialized() {
    let source = r#"
struct Box { value: i64 }
...
#[test]
fn check_allows_non_copy_field_move_from_owned_local() {
    let source = r#"
...
#[test]
fn check_rejects_use_of_base_after_field_move() {
    let source = r#"
struct Inner { value: i64 }
struct Holder { inner: Inner }
...
"#;
    let file = write_temp_paco("field_move_reuse", source);
    let cli = paco_driver::Cli::try_parse_from(["paco", "check", file.to_str().unwrap()]).unwrap();
...
#[test]
fn check_rejects_non_copy_field_move_through_borrow() {
    let source = r#"
struct Inner { value: i64 }
struct Holder { inner: Inner }
...
"#;
    let file = write_temp_paco("field_move_through_borrow", source);
    let cli = paco_driver::Cli::try_parse_from(["paco", "check", file.to_str().unwrap()]).unwrap();
