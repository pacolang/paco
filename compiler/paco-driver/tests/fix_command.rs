...
//! local git repository standing in for one of `github.com/pacolang/tensor`,
//! `github.com/pacolang/math` or `github.com/pacolang/blas`, pre-fetched
//! into the package cache the way a prior `paco get` would have left it —
//! `paco fix` itself only edits `paco.mod`/sources; it does not fetch).
...
/// The fixed placeholder version `paco fix` writes into `paco.mod` for
/// any of the three moved libraries, tagged on each throwaway repo below,
/// while no real tag has been cut for any of them yet
/// (`extract-domain-libraries` design.md's "latest tag compatible with
/// the running compiler" note).
const LIBRARY_TAG: &str = "v0.1.0";
...
fn throwaway_repo(name: &str, module_file_name: &str, module_source: &str) -> PathBuf {
    let dir = temp_dir(name);
    git(&dir, &["init", "--quiet"]);
    git(&dir, &["config", "user.email", "test@example.com"]);
    git(&dir, &["config", "user.name", "Test"]);
    fs::create_dir_all(dir.join("src")).unwrap();
    fs::write(dir.join("src").join(module_file_name), module_source).unwrap();
    git(&dir, &["add", "."]);
    git(&dir, &["commit", "--quiet", "-m", "init"]);
    git(&dir, &["tag", LIBRARY_TAG]);
    dir
}

/// A throwaway local git repository standing in for `github.com/pacolang/tensor`.
fn throwaway_tensor_repo(name: &str) -> PathBuf {
    throwaway_repo(name, "tensor.paco", FAKE_TENSOR_MODULE)
}

/// A throwaway local git repository standing in for `github.com/pacolang/math`.
fn throwaway_math_repo(name: &str) -> PathBuf {
    throwaway_repo(name, "math.paco", FAKE_MATH_MODULE)
}
...
/// Pre-fetches `repo` into `cache_root` at `module_path` under
/// `LIBRARY_TAG` and writes a matching (but not-yet-declared-in-
/// `paco.mod`) lock entry — `resolve_domain_use_path` looks lock entries
/// up by dependency path and tag alone, so this is harmless until `paco
/// fix` adds the same entry to `paco.mod`, exactly matching what a real
/// prior `paco get` followed by `paco fix` would leave behind.
fn prefetch_library(module_path: &str, repo: &std::path::Path, cache_root: &std::path::Path, program_dir: &std::path::Path) {
    let fetched = pkg_cache::fetch_into_cache(cache_root, module_path, LIBRARY_TAG, RefKind::TagOrBranch, repo.to_str().unwrap(), None).unwrap();
    let mut lockfile = manifest::Lockfile::read(&program_dir.join("paco.lock")).unwrap_or(manifest::Lockfile { packages: Vec::new() });
    lockfile.packages.push(manifest::LockedPackage {
        path: module_path.to_string(),
        tag: Some(LIBRARY_TAG.to_string()),
        commit: fetched.commit,
        ..Default::default()
    });
    lockfile.write(&program_dir.join("paco.lock")).unwrap();
}

#[test]
fn fix_rewrites_an_aliased_import_and_adds_the_dependency() {
    let repo = throwaway_math_repo("aliased_repo");
    let cache_root = temp_dir("aliased_cache");
    let program_dir = temp_dir("aliased_program");
    prefetch_library("github.com/pacolang/math", &repo, &cache_root, &program_dir);
    ...
    assert_eq!(fixed, "use github.com/pacolang/math as m;\n\nfn main() {\n    print(m::value())\n}\n");
    ...
    assert_eq!(parsed.dependencies, vec![("github.com/pacolang/math".to_string(), manifest::DependencySource::Tag(LIBRARY_TAG.to_string()))]);
...
fn fix_rewrites_a_numerics_import_the_qualifier_and_the_add_call() {
    let repo = throwaway_tensor_repo("numerics_repo");
    let cache_root = temp_dir("numerics_cache");
    let program_dir = temp_dir("numerics_program");
    prefetch_library("github.com/pacolang/tensor", &repo, &cache_root, &program_dir);
    fs::write(program_dir.join("paco.mod"), "module = \"example.com/myprogram\"\n").unwrap();
    fs::write(
    ...
    let fixed = fs::read_to_string(program_dir.join("main.paco")).unwrap();
    assert!(fixed.starts_with("use github.com/pacolang/tensor;\n"), "{fixed}");
...
#[test]
fn fix_adds_a_dependency_for_each_moved_module_a_file_actually_uses() {
    let tensor_repo = throwaway_tensor_repo("multi_tensor_repo");
    let math_repo = throwaway_math_repo("multi_math_repo");
    let cache_root = temp_dir("multi_cache");
    let program_dir = temp_dir("multi_program");
    prefetch_library("github.com/pacolang/tensor", &tensor_repo, &cache_root, &program_dir);
    prefetch_library("github.com/pacolang/math", &math_repo, &cache_root, &program_dir);
    fs::write(program_dir.join("paco.mod"), "module = \"example.com/myprogram\"\n").unwrap();
    fs::write(
        program_dir.join("main.paco"),
        "use stdlib::numerics;\nuse stdlib::math;\n\nfn main() {\n    let t = numerics::Tensor<f32, 2, 2>::zeros();\n    print(t.len() + math::value())\n}\n",
    )
    .unwrap();
    ...
    let manifest_text = fs::read_to_string(program_dir.join("paco.mod")).unwrap();
    let parsed = manifest::Manifest::parse(&manifest_text).unwrap();
    assert_eq!(parsed.dependencies.len(), 2, "{manifest_text}");
    assert!(
        parsed.dependencies.contains(&("github.com/pacolang/tensor".to_string(), manifest::DependencySource::Tag(LIBRARY_TAG.to_string()))),
        "{manifest_text}"
    );
    assert!(
        parsed.dependencies.contains(&("github.com/pacolang/math".to_string(), manifest::DependencySource::Tag(LIBRARY_TAG.to_string()))),
        "{manifest_text}"
    );

    let fixed = fs::read_to_string(program_dir.join("main.paco")).unwrap();
    assert!(fixed.contains("use github.com/pacolang/tensor;"), "{fixed}");
    assert!(fixed.contains("use github.com/pacolang/math;"), "{fixed}");

    let run = paco().arg("run").arg(program_dir.join("main.paco")).env("PACO_PKG_CACHE", &cache_root).output().unwrap();
    assert!(run.status.success(), "paco run failed after `paco fix`: {}", String::from_utf8_lossy(&run.stderr));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "13\n");

    let _ = fs::remove_dir_all(&tensor_repo);
    let _ = fs::remove_dir_all(&math_repo);
    let _ = fs::remove_dir_all(&cache_root);
    let _ = fs::remove_dir_all(&program_dir);
}
...
fn fix_is_idempotent() {
    let repo = throwaway_tensor_repo("idempotent_repo");
