...
fn check_reports_struct_type_error() {
    let source = "struct Point { x: i64 } fn make() -> Point { Point { x: true } }";
...
fn check_reports_undeclared_name_inside_struct_literal_field() {
    let source = "struct Point { x: i64 } fn make() -> Point { Point { x: missing } }";
...
fn check_reports_undeclared_name_inside_struct_method() {
    let source = "struct Point { x: i64, fn bad(&self) -> i64 { missing } }";
