...
#[test]
fn type_checker_checks_struct_pattern_fields() {
    ...
    assert!(check_source(&program("Point { x, .. }")).is_none());
    assert!(check_source(&program("Point { x, y: _ }")).is_none());
    let missing = check_source(&program("Point { x }")).expect("missing field without `..` is rejected");
    assert!(missing.contains("does not mention field `y`"), "{missing}");
    let unknown = check_source(&program("Point { x, z: 1, .. }")).expect("unknown field is rejected");
    assert!(unknown.contains("unknown field `z`"), "{unknown}");
}
