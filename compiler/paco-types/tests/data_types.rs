...
fn type_checker_accepts_struct_literal_field_access() {
    let error = check_source(
        "struct Point { x: i64, y: i64 } fn main() { let p = Point { x: 1, y: 2 }; print(p.x) }",
    );
...
fn type_checker_rejects_wrong_struct_field_type() {
    let error = check_source(
        "struct Point { x: i64, y: i64 } fn main() { let p = Point { x: true, y: 2 }; }",
    )
...
fn type_checker_rejects_missing_and_unknown_struct_fields() {
    let missing =
        check_source("struct Point { x: i64, y: i64 } fn main() { let p = Point { x: 1 }; }")
            .expect("expected missing field error");
    let unknown =
        check_source("struct Point { x: i64 } fn main() { let p = Point { x: 1, y: 2 }; }")
            .expect("expected unknown field error");
    let duplicate =
        check_source("struct Point { x: i64 } fn main() { let p = Point { x: 1, x: 2 }; }")
...
fn type_checker_accepts_enum_variant_construction() {
    let error =
        check_source("enum Maybe { Some(i64), None } fn main() { let x: Maybe = Maybe::Some(1); }");
...
fn type_checker_instantiates_generic_struct_field() {
    let error = check_source(
        "struct Box<T> { value: T } fn main() { let b = Box<i64> { value: 1 }; print(b.value) }",
    );
...
fn type_checker_instantiates_generic_function_calls() {
    let valid = check_source("fn id<T>(value: T) -> T { value } fn main() { let x: i64 = id(1); }");
    let invalid =
        check_source("fn id<T>(value: T) -> T { value } fn main() { let x: i64 = id(true); }")
...
fn type_checker_rejects_generic_arity_mismatches_in_declarations() {
    ...
    let extra =
        check_source("struct Box<T> { value: T } fn bad(value: Box<i64, bool>) {} fn main() {}")
...
fn type_checker_rejects_unknown_methods_and_associated_calls() {
    let method =
        check_source("struct Point { x: i64 } fn main() { let p = Point { x: 1 }; p.y() }")
            .expect("expected unknown method error");
    let associated = check_source("struct Point { x: i64 } fn main() { let p = Point::origin(); }")
...
fn type_checker_rejects_immutable_field_assignment() {
    let error =
        check_source("struct Point { x: i64 } fn main() { let p = Point { x: 1 }; p.x = 2 }")
...
#[test]
fn a_variant_payload_mentioning_a_same_named_parameter_does_not_capture_it() {
    let error = check_source(
        "
...
struct Tensor<T, U> {
    ...
    fn make() -> Result<Self, i64> {
        Result::Ok(Tensor<T, U> { data: slice_of_zeros<T>(3) })
    }
}
fn main() {}
",
...
#[test]
fn a_bare_unit_variant_joins_an_if_with_a_constructed_variant() {
    let error = check_source(
        "enum Maybe<T> { Some(T), None } fn pick(flag: bool) -> Maybe<string> { if flag { Some(\"one\") } else { None } }",
    );
