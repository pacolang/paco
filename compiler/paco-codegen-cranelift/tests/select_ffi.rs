//! Confirms `paco-codegen-cranelift` compiles `select` (task 5's
//! readiness-check lowering) into working calls against the real
//! `paco-runtime-ffi` functions — JIT-linked directly, matching
//! `channel_ffi.rs`/`spawn_ffi.rs`'s pattern.
...
#[test]
fn select_picks_the_ready_arm_over_default() {
...
#[test]
fn select_runs_default_when_nothing_is_ready() {
    let output = compile_and_run_main(
        r#"
        fn main() -> i64 {
            let (tx, rx) = channel<i64>(capacity: 1);
