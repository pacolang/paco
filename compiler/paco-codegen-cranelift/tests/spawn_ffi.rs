//! Confirms `paco-codegen-cranelift` compiles `spawn`/`.join()` (task 4's
//! thunk-outlining lowering) into working calls against the real
//! `paco-runtime-ffi` functions — JIT-linked directly against this test
//! binary's own copy of them, matching `channel_ffi.rs`'s pattern.
...
#[test]
fn a_spawned_task_runs_and_its_result_is_observable_through_join() {
...
#[test]
fn a_spawned_task_captures_an_outer_variable() {
    let output = compile_and_run_main(
        r#"
        enum Result { Ok(i64), Err(TaskPanic) }
...
#[test]
fn a_spawned_producer_sends_over_a_channel_captured_by_move() {
    let output = compile_and_run_main(
        r#"
