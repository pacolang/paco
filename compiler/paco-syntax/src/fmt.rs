...
use crate::parse::is_block_like;
use crate::ast::{
    AssocTypeDecl,
    BinaryOp, Block, ConstDecl, EnumDecl, Expr, ExternBlock, FnDecl, GenericParam, GenericParamKind, Item, Literal,
...
/// Renders one standalone `Item` (no `source`-based comment/whitespace
/// preservation) — used to pretty-print a `phase-9-comptime` `Code` value's
/// spliced content, e.g. for tests and future diagnostics referencing
/// generated code.
...
/// Renders one standalone `Expr`, for the same `Code`-value use case as
/// [`format_item`].
...
impl<'a> Formatter<'a> {
    ...
    fn format_item(&mut self, item: &Item) {
        match item {
            ...
            Item::Const(const_decl) => self.format_const_decl(const_decl),
            Item::Extern(extern_block) => self.format_extern_block(extern_block),
        }
    }
    ...
    fn format_extern_block(&mut self, extern_block: &ExternBlock) {
        ...
        for signature in &extern_block.functions {
            self.write_indent();
            self.write("fn ");
            self.write(&signature.name);
            self.write("(");
            ...
            self.write(")");
            if let Some(ref ret_ty) = signature.return_ty {
                self.write(" -> ");
    ...
    fn format_const_decl(&mut self, const_decl: &ConstDecl) {
        self.write_indent();
        self.write("const ");
        self.write(&const_decl.name);
        self.write(": ");
        self.format_ty(&const_decl.ty);
        self.write(" = ");
    ...
    fn format_generics(&mut self, generics: &[GenericParam]) {
        ...
        for (i, param) in generics.iter().enumerate() {
            ...
            match &param.kind {
                ...
                GenericParamKind::Dim => {
                    self.write("dim ");
                    self.write(&param.name);
                }
                GenericParamKind::Const(ty) | GenericParamKind::ConstPack(ty) => {
                    self.write("const ");
                    self.write(&param.name);
                    self.write(": ");
                    self.format_ty(ty);
    ...
    fn format_fn_decl(&mut self, fn_decl: &FnDecl) {
        ...
        if let Some(abi) = &fn_decl.extern_abi {
            self.write("extern \"");
            self.write(abi);
            self.write("\" ");
        }
        self.write("fn ");
        match &fn_decl.name_splice {
            Some(splice) => {
                self.write("#(");
                self.format_expr(splice);
                self.write(")");
            }
            None => self.write(&fn_decl.name),
        }
    ...
    fn format_struct_decl(&mut self, struct_decl: &StructDecl) {
        ...
        for constant in &struct_decl.consts {
            self.format_const_decl(constant);
            self.newline();
        }
        self.format_assoc_types(&struct_decl.assoc_types);
    ...
    fn format_assoc_types(&mut self, assoc_types: &[AssocTypeDecl]) {
        for assoc_type in assoc_types {
            self.write_indent();
            self.write("type ");
            self.write(&assoc_type.name);
            if let Some(default) = &assoc_type.default {
                self.write(" = ");
                self.format_ty(default);
            }
            self.write(";");
            self.newline();
        }
    ...
    fn format_enum_decl(&mut self, enum_decl: &EnumDecl) {
        ...
        for constant in &enum_decl.consts {
            self.format_const_decl(constant);
            self.newline();
        }
    ...
    fn format_trait_decl(&mut self, trait_decl: &TraitDecl) {
        ...
        for (i, method) in trait_decl.methods.iter().enumerate() {
            ...
            self.format_generics(&method.generics);
            self.write("(");
        ...
        for constant in &trait_decl.consts {
            self.format_const_decl(constant);
            self.newline();
        }
        self.format_assoc_types(&trait_decl.assoc_types);
    ...
    fn format_methods_block(&mut self, methods_block: &MethodsBlock) {
        ...
        for constant in &methods_block.consts {
            self.format_const_decl(constant);
            self.newline();
        }
    ...
    fn format_block(&mut self, block: &Block) {
        ...
        for (i, stmt) in block.stmts.iter().enumerate() {
            let is_last = i + 1 == block.stmts.len() && block.tail.is_none();
            self.format_stmt(stmt, is_last);
        }
    ...
    fn format_stmt(&mut self, stmt: &Stmt, is_last: bool) {
        match stmt {
            Stmt::Let(let_stmt) => {
                self.write_indent();
                self.write("let ");
    ...
    fn format_pat(&mut self, pat: &Pat) {
        match pat {
            ...
            Pat::Struct { path, fields, rest, .. } => {
                ...
                if *rest {
                    self.write(if fields.is_empty() { ".." } else { ", .." });
                }
                self.write(" }");
            }
    ...
    fn format_ty(&mut self, ty: &Ty) {
        match ty {
            ...
            Ty::RawPointer { mutable, ty: inner, .. } => {
                self.write(if *mutable { "*mut " } else { "*const " });
                self.format_ty(inner);
            }
            Ty::Dyn { trait_path, .. } => {
                self.write("dyn ");
            ...
            Ty::Const(expr, _) => self.format_expr(expr),
            Ty::DynDim(_) => self.write("Dyn"),
            Ty::Existential(name, _) => {
                self.write("?");
                self.write(name);
            }
            Ty::Expand(name, _) => {
    ...
    fn format_expr(&mut self, expr: &Expr) {
        match expr {
            ...
            Expr::Call { callee, args, .. } => {
                self.format_expr_with_precedence(callee, 12);
                self.write("(");
                for (i, arg) in args.iter().enumerate() {
                    if i > 0 {
                        self.write(", ");
            ...
            Expr::MethodCall { receiver, method, args, .. } => {
                self.format_expr_with_precedence(receiver, 12);
                self.write(".");
                self.write(method);
                self.write("(");
                for (i, arg) in args.iter().enumerate() {
            ...
            Expr::Unary { op, expr, .. } => {
                ...
                self.format_expr_with_precedence(expr, 11);
            }
            ...
            Expr::Field { base, field, .. } => {
                self.format_expr_with_precedence(base, 12);
                self.write(".");
                self.write(field);
            }
            Expr::Index { base, index, .. } => {
                self.format_expr_with_precedence(base, 12);
                self.write("[");
                for (i, index_expr) in index.iter().enumerate() {
                    if i > 0 {
                        self.write(", ");
                    }
                    self.format_expr(index_expr);
                }
                self.write("]");
            }
            Expr::Try { expr, .. } => {
                self.format_expr_with_precedence(expr, 12);
                self.write("?");
            }
            Expr::Cast { expr, ty, .. } => {
                self.format_expr_with_precedence(expr, 11);
            ...
            Expr::Quote(body, _) => {
                self.write("quote {");
                match body.as_ref() {
                    QuoteBody::Item(item) => self.format_item(item),
                    QuoteBody::Expr(expr) => self.format_expr(expr),
                }
                self.write("}");
            }
            Expr::Splice(expr, _) => {
